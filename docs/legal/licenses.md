...
| Fonts: IBM Plex Sans (Regular/Medium/SemiBold), IBM Plex Mono (Regular/Medium) | Plex Sans 3.005, Plex Mono 2.005, from the official IBM/plex GitHub releases (`fonts/complete/ttf` + `woff2`) | SIL OFL 1.1 | IBM | Bundled in `apps/mobile/assets/fonts` (native, expo-font) and `apps/mobile/public/fonts` (web + desktop, `@font-face`, same origin only) with `LICENSE-OFL.txt` beside the files; attribution; may not be sold on their own |
