...
| Fonts: IBM Plex Sans (Regular/Medium/SemiBold), IBM Plex Mono (Regular/Medium) | Plex Sans 1.1.0, Plex Mono 2.5.0, from the official IBM/plex GitHub releases (`fonts/complete/ttf` + `woff2`) | SIL OFL 1.1 | IBM | Bundled in `apps/mobile/assets/fonts` (native, expo-font) and `apps/mobile/public/fonts` (web + desktop, `@font-face`, same origin only) with `LICENSE-OFL.txt` beside the files; attribution; may not be sold on their own |
| Icons: Lucide (14 vector paths) | lucide main, 6.9.2026, copied into `packages/ui/src/icons/paths.ts` | ISC | Lucide Contributors (portions Feather, Cole Bemis, MIT) | Keep the notice `packages/ui/src/icons/LICENSE-lucide.txt`; no other obligation |
