...
- **Contrast.** The app's colour tokens are fixed so that body, secondary and tertiary text reach a contrast ratio of at least **4.5:1** on every surface they are allowed to sit on, in the dark theme and in the light theme, and so does the text on the primary button. An automated test in our test suite asserts those ratios and fails when a colour is changed to something that breaks them. It does not yet cover every coloured element; section 5 names the one we have measured as failing.
...
- **The delete and wipe buttons fail the contrast rule in the dark theme.** Their text sits at about **3.1:1** on the red fill, below the 4.5:1 we hold the rest of the app to, and about 4.3:1 in the light theme. The automated contrast test does not currently cover that colour pair, which is how it survived.
- **You cannot send a message from a hardware keyboard.** On a tablet or in a browser with a physical keyboard, Return inserts a newline or does nothing, and there is no key that sends. Sending needs a tap or a click. This is the most serious gap in this list for anyone who cannot use a touchscreen.
