...
| 3. Play purchase sheet + Restore on the OnePlus 11 | DONE on versionCode 2: real Play sheet, test purchase → "YOU OWN PRO", order Processed, fresh reinstall + Restore → "Purchase restored", AT-1 proof captured |
| 4. ASC products check | DONE: ids/prices right, 6 locales, review screenshots uploaded → all four **READY_TO_SUBMIT** |
...
6. **Fix build (versionCode 2):** branch synced with main (`fb9c532`, includes the key fix `8054512` and the products script), release AAB built in the `purchases-verify` worktree with `INBORN_PACKS=instant,fast INBORN_VERSION_CODE=2` and the upload key (`./gradlew --no-daemon bundleRelease -PreactNativeArchitectures=arm64-v8a`; the first two attempts died in `:app:signReleaseBundle`, once from a foreign `gradlew --stop`, once from the known low-memory flake, and the signing task passed on rerun). Gates: `bundletool validate` OK, manifest `versionCode="2" versionName="1.0.0"`, `inborn_model` fast-follow, `check-android-permissions.sh` "OK: no INTERNET permission". AAB 1,870,261,312 bytes, sha256 `96d625818bab0f93b64adc3eed38bd75c879ad8b0c80e0ff99ec08cfe29c3643`. Uploaded with `scripts/play-upload.mjs` as edit `05971046557238491861`, internal release "1.0.0 (2) internal, purchase proof", status completed.

7. **versionCode 2 on the phone (7.9.2026 08:24–08:45):** Play offered "Update" 7 minutes after the upload; installed (`versionCode=2`, installer `com.android.vending`), Play re-delivered the Instant pack (`assetpacks/inborn_model/2/2/…`). New paywall (work-tier merged): Pro ₪60.00, Work ₪199.90, Restore purchases (`op11/31-v2-paywall-before.png`).
   - The first order (GPA.3381…) shows **Refunded ILS 0.00** in Order management: Play refunds an unacknowledged licence-tester purchase within minutes, so v1's rejection cost the purchase itself; Restore on v2 correctly found nothing.
   - **Purchase on v2:** Unlock Pro → Play sheet "Inborn Pro ₪60.00 · Test card, always approves · test order, not charged" (`op11/32-v2-sheet.png`) → 1-tap buy → the paywall switched to **"YOU OWN PRO · Unlocked on every device that uses this Google Play account"** and the Work card became **"Upgrade to Work · ₪149.90"** (the `inborn.work.upgrade` SKU for Pro owners) (`op11/34-v2-after-buy.png`). Order **GPA.3398-9587-5229-18767, Processed ILS 60.00** (test), not refunded → the app verified and acknowledged it (`op11/44-orders-console.png`).
   - **Restore (T21):** `pm clear` is refused on this phone (SecurityException, no CLEAR_APP_USER_DATA for shell on Android 16), so the clean-slate proof was uninstall → reinstall from the Play Store (20 s) → open paywall: **"YOU OWN PRO" immediately** (startup refresh) and "Restore purchases" → toast **"Purchase restored"** (`op11/41-fresh-install-paywall.png`, `42-fresh-restore-3s.png`, `43-fresh-after-restore.png`).
   - **AT-1 release-build proof:** Proof screen (`inborn://proof`): **"SEALED · ON-DEVICE", "OUT 0 B · IN 0 B", "Internet: none (not in the manifest)", "NETWORK ALLOWLIST: none · the app has no internet permission", "CONNECTIONS 0 open"** (`op11/35-proof-screen.png`). Airplane test with Airplane Mode really on (`cmd connectivity airplane-mode enable`, status bar shows the airplane icon; restored to off right after): the on-device INSTANT model answered "17 times 23 equals 391." and the counter stayed **OUT 0 B · IN 0 B** (`op11/37-airplane-answer.png`).
   - **Bug B-AT1-1 (medium):** on that screen the app's own indicator said "OFF" and the footer "Airplane Mode is off; the counter still shows OUT 0 B", because `useConnectivity` derives "offline" from `getNetworkStateAsync().isConnected` and the OnePlus keeps Wi-Fi connected under Airplane Mode. Fix on this branch (`apps/mobile/src/proof/connectivity.ts`): on Android also read `Network.isAirplaneModeEnabledAsync()` and treat it as offline. Typecheck + eslint clean; device verification rides with versionCode 3.
...
- **Review screenshots (7.9.2026):** the real Android paywall captures (`op11/31`, `op11/34` for the upgrade SKU) uploaded through the API (`inAppPurchaseAppStoreReviewScreenshots` create → PUT parts → PATCH uploaded+md5). Apple rejected 1080×2412 PNG/JPEG with `IMAGE_INCORRECT_DIMENSIONS`; scaled to 2796 px tall and padded to **1290×2796** JPEG it processed to COMPLETE and all four products moved from `MISSING_METADATA` to **`READY_TO_SUBMIT`**. (Earlier note, kept for the record:) every product was `MISSING_METADATA` because it had no App Store review screenshot (`appStoreReviewScreenshot` = null). Apple requires one real in-app screenshot per IAP (reviewer-only, not public). The composed store screenshots in `design/store/out/*/04-paywall.png` are marketing frames, not raw captures; the plan is to upload the real paywall screenshot taken during the iPhone step (section 5) through the API (`inAppPurchaseAppStoreReviewScreenshots` create → upload → commit). Also: the app version 1.0 is PREPARE_FOR_SUBMISSION, and the IAPs are submitted with the first app version (Moshe's action).
...
4. Submitting the four IAPs (all READY_TO_SUBMIT) with app version 1.0.
5. Decide whether Play regional prices stay on the store conversion or move to the country-ratio table (spec §12.2).

## 7. Devices, processes

OnePlus 11: only `com.inbornapp.mobile` installed/uninstalled/reinstalled (the Play-installed versionCode 2 build is left in place, Pro owned by owner@example.com); Airplane Mode was toggled on for the AT-1 test and restored to off; "Require authentication for purchases?" answered "No, thanks" (no change); phone left on the home screen. Safari: five short-lived windows of my own for the console, each closed; no other window or tab touched. No emulator or server started.
