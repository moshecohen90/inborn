...
| S. Play internal release versionCode 16 — the submission candidate until vc17 superseded it the same evening (22.9) | DONE: AAB **5,117,817,952 bytes** from `main` 9da93a2 with `INBORN_PACKS` unset, all seven packs, no INTERNET, `bundletool validate` rc 0, uploaded as "1.0.0 (16)" (edit 01823738192199699579); Play updated the 6T in place vc15 → vc16 in 665 s with **every pack intact and zero install- nodes in the vault**; About 1.0.0 (16) / 9da93a296bbe, Proof OUT 0 B; **real Play billing: YOU OWN PRO, Work upgrade ₪149.90, Restore → "Purchase restored"**; one hour of continuous use in soak run 9 |
| T. Play internal release versionCode 17 — the Android submission candidate (22.9) | DONE: AAB **5,117,857,219 bytes** from `main` 239a268 with `INBORN_PACKS` unset, all seven packs, no INTERNET, `bundletool validate` rc 0, uploaded as "1.0.0 (17)" (edit 02581446358610636994, **second attempt** — the first edit died after `:commit` answered 400 for eleven minutes holding a bundle Play had already hashed correctly); Play updated the 6T in place vc16 → vc17 in 1,340 s on the **first** press, every pack re-delivered and **zero install- nodes in the vault**; About 1.0.0 (17) / 239a268a082b, Proof OUT 0 B; **real Play billing: YOU OWN PRO, Work upgrade ₪149.90, Restore → "Purchase restored"**; §5.7 incognito passes paired on real hardware; soak run 10 is an hour with 11 clean F33 cycles, one process for 56 minutes and no crash in a 71,932-line buffer. No emulator half — that device belonged to another stream, so vc16’s device-tier rows are not restated here |
...
| S7 | Fast's own speed claim against the measurement | the FAST card on this phone reads `1.2 GB · Q4_K_M · Battery: Medium`, **`~5-7 tok/s on your phone`**, `Loaded`, `In use`, `RECOMMENDED ON THIS PHONE · CHAT IN ENGLISH`. Soak run 9 measured Fast over four turns at **6.1 / 6.2 / 6.6 / 7.0 tok/s** — min 6.1, median 6.4, max 7.0, the whole spread inside the card's range and touching its ceiling exactly. The range is `speed.ts`'s `android-legacy` row (`fast: [5, 7]`), a file byte-identical between vc16's source 9da93a2 and current `origin/main`. **The card does not overstate, so this is not a finding.** Instant's measured 14.1–14.2 likewise sits inside its `[12, 18]` row | `b-01-vault-top.png`, `docs/qa/soak-run-9-2026-09-22.md` |
