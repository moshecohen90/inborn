...
with the reason. Four findings of this run are fixed on this branch (F160–F163). Two rows were measured **FAIL** at
the start of the run and **PASS** at the end of it, because the `responsive-design` stream (round 35, F110–F119)
landed while it was going; both were re-measured with the same driver on the merged build rather than taken on trust.
...
|---|---|---|---|
| 1 | "on a big screen the onboarding shows a very, very long button… designed for mobile, stretched on desktop, it does not look good. Check the whole UI fits every device size" | **FAIL → PASS**, re-measured after round 35 landed mid-run | **Before** (this round's first pass, on `origin/main` at the time): the primary button was **1408 × 48 px at a 1440 viewport, 98 % of the width**, and 992 px (97 %) at 1024, while the download door above it was already capped at 398 px. **After** the `responsive-design` merge, same driver, same four widths: **446 px at 768, 1024 and 1440** (31 % of a 1440 window), centred in a card, and 358 px at 390 unchanged. `web/a1-onboarding.json` holds the after numbers; the before ones are quoted here because the same file was overwritten by the re-run. Pictures: `web/A1-02-welcome-1440.png`, `web/A1-03-model-1440.png` |
...
| 8 | "the input with + on the left and send on the right is not at the same height… only on big screens, because in the app the field is one line and here it is 2" | **FAIL → PASS**, re-measured after round 35 landed mid-run | **Before:** the field was a **fixed 70 px box** at every width beside a 44 px `+` and a 36 px send — 70 px for one line of text, and it did not grow for a second, which is exactly the two-line look he described on an empty composer. **After:** the empty field is **45 px** and the three controls share one centre line to the pixel — `+` centreY 869, field centreY 869, send centreY 869 — at 390, 768, 1024 and 1440; typing a second line grows the field to 95 px and the buttons stay on the bottom row. `web/a5-ui.json` (`composer`, `composerTwoLines`), `web/A5-00-chat-1440.png`, `web/A5-01-composer-2lines-1440.png` |
...
| 11 | "on Android and iPhone I attached a photo of a door and it said it received no image… I sent a PDF and the same problem" | **PASS on both phones** (rounds 37 and 38, F125–F129 and F135–F144) | Android: `docs/qa/attach-android/f125-photo-button-sees-after.png` — *"I see a brown door with a gold coin on the right side."*; the file answered around before its index existed is `f126-invented-while-indexing-before.png` → `f126-cited-after.png`. iPhone: `docs/qa/attach-ios/M8-photo-with-doc.png`, `M11b-answer.png`. Both streams finished on the real devices this evening; this round cites them rather than repeating them — `android/README.md`, `ios/README.md` |
...
| Excel and HTML import (Work only) | — | `sites.xlsx` and `quarterly-note.html` both stop at the paywall: "You opened a spreadsheet. Pro for Work reads Excel and HTML files." | **proven on the 6T** | **PASS.** Free and Pro halves in the browser (`web/a3-matrix-pro.json`); the Work half on the phone in round 37: `indexed · 1 chunk`, answer *"…is BR-3311-LM."* cited `bearings.xlsx · sheet 1`, and the HTML note cited `sitenotes.html · part 2` — `docs/qa/attach-android/tier-work-xlsx-cited.png`, `tier-work-html-cited.png`. The `dist-dev-work` export was built here but not driven |
| Folders and full export | locked, paywall says "Folders for your chats come with Pro." | — | — | **PASS on the Free half** `web/a5-ui.json` |
| Text from scans and photos (OCR) | — | **proven on the 6T** | — | The browser tier ships no OCR at all (`apps/mobile/src/documents/extract.ts`: "No OCR in the browser tier yet"), so this row lives on a device: round 37 imported a rendered scan `needs-ocr · 0 chunks`, ran **Run OCR** → `indexed · 1/1 pages · 1 chunks · 2706 ms`, and the strict answer *"…is WP-8832-VN."* cited `p.1` — `docs/qa/attach-android/tier-pro-ocr-indexed.png`, `tier-pro-ocr-cited.png` |
...
| Redaction before sending (Work) | — | — | **proven on the 6T** | Round 37: name list + Dates → preview, applied to the composer as `[NAME-1]` / `[DATE-1]`, the sent message marked **REDACTED** with *Show originals* — `docs/qa/attach-android/tier-work-redact-preview.png`, `tier-work-redact-applied.png`, `tier-work-redacted-sent.png` |
| Client vaults, per-vault audit log, signed records | — | — | — | **not verified** on this run. Work's own screens (`/work/verify`, `/work/audit`, `/work/statement`) were only reached through the F162 fix below |
| Real prices on the store | — | — | — | **proven on the 6T** on 22.9 through real Play billing: *YOU OWN PRO*, Work upgrade ₪149.90, *Purchase restored* — `docs/qa/purchases-run-2026-09-11.md` §S, §T. No sandbox purchase was made in this round either |
...
| Work's client vaults, per-vault audit log, signed records | not driven this round |
| F160–F163 on the phones | the fixes were written after the builds those phones carry; putting them there means replacing the app Moshe's own data lives in. Proven in a real browser over the identical `packages/core` and `apps/mobile/src`, with unit tests that run on every platform — `android/README.md`, `ios/README.md` |
