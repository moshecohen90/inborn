# Round 51 — design review fixes (F240–F254), 24.9.2026
...
| `*.log` | the measurements quoted below: `measure-*` (touch targets, contrast, horizontal scroll per route), `dim-*` (effective contrast of every dimmed line), `center-*` (nav-title centre vs viewport centre per locale), `strip-*`, `sweep-*`, `interact-*`, `onboard-*` |

## What the measurements say

**Touch targets, 390, dark** (`measure-before-390-dark.log` → `measure-after-390-dark.log`). Before: 19 segmented
...
**Dimmed rows** (`dim-before-*.log` → `dim-after-*.log`). Before: three failures in dark (2.76:1) and six in light,
...
clears 4.5:1 for `text2` on `well` (0.5 → 2.22, 0.7 → 3.27, 0.85 → 4.55 — `apps/mobile/test/fixes-r51.test.ts`
pins the arithmetic).

**Documents nav title** (`center-before.log` → `center-after.log`), title centre vs the 195 px viewport centre at 390:
...
**The empty chat** (`strip-after.log`): the headline was 605 px down an 844 px viewport (72 %); it is now 501 and the
...
and so are all twelve pages at both widths. `apps/site/check.mjs` measures it on every run and fails with the file
and the offending elements; reverting the one CSS line turns it red at both widths.
...
| F242 | disabled rows fail contrast | `before/390-light--settings.png`, `dim-before-light.log` | `after/390-light--settings.png`, `dim-after-light.log` |
| F243 | touch targets under 44 | `measure-before-390-dark.log` | `measure-after-390-dark.log`, `after/390-dark--settings.png` |
...
| F245 | Documents title off-centre | `center-before.log` | `center-after.log`, `after/390-dark--documents.png` |
...
source and locale changes proven by `apps/mobile/test/fixes-r51.test.ts` and by the site gate; they show up inside
