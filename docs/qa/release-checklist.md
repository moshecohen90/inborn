...
- 11.9.2026 iPhone 13 Pro, zero hands (`docs/qa/voice-run-2026-09-11.md`): permission alert answered by the runner; iOS refused with "Assets are not installed, Siri or Dictation is disabled" (on-device English dictation is off on that phone), the offline-missing sheet + Whisper·PRO shown as specified; the first-ever tap after Allow ends in `interrupted` (open fix: retry once after a grant). Real spoken words through the system engine still need the phone's dictation setting on.
