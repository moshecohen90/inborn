...
import { Bm25Index, buildRagPrompt, DEFAULT_MIN_COSINE_ALONE, isRelevant, type DocumentRecord, type RetrievalHit, relevanceDoors } from "../src/rag";
...
const EXPECTED_BY_LANG: Record<string, string> = {
  de: "5/5",
  "de (no accents)": "4/4",
  /* 8/10 before F365: "Which cities host the offices?" shares one word at cosine 0.8098, under the 0.815 door. */
  en: "7/10",
  es: "3/5",
  "es (no accents)": "2/4",
  fr: "6/6",
...
const EXPECTED_LEXICAL_BY_LANG: Record<string, string> = {
  de: "0/5",
  "de (no accents)": "2/4",
  /* F365 took one each from en, zh and zh-Hant: "offices" under 0.815, and "前一年" whose only shared word is a date. */
  en: "3/10",
  es: "1/5",
  "es (no accents)": "2/4",
  fr: "1/6",
...
describe("F334 · one-passage documents in nine languages", () => {
  for (const strict of [false, true]) {
    it(`strict=${strict}: 80 of 89 on-topic questions cite the passage and 0 of 143 off-topic ones do`, () => {
      ...
      expect([on.length, on.filter(Boolean).length]).toEqual([89, 80]);
  ...
  it("F367: per language, what the lexical rule cites on its own", () => {
    ...
    for (const d of ONE)
      for (const q of d.questions.filter((x) => x.kind === "on")) {
        ...
        const lexical = isRelevant(h, { ...relevanceDoors("embed-e5"), alone: Infinity });
        const cur = (byLang[labelOf(d.lang, q)] ??= [0, 0]);
        cur[0] += lexical ? 1 : 0;
        cur[1]++;
      }
