...
describe("expected speed (spec §6.4)", () => {
  it("classes devices by platform and RAM and always answers with a range", () => {
    ...
    expect(chipClassFor({ os: "android", ramGB: 8, chipName: "Snapdragon 845" })).toBe("android-legacy");
    expect(chipClassFor({ os: "android", ramGB: 12, chipName: "Snapdragon 8 Gen 1" })).toBe("android-legacy");
