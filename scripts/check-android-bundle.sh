...
for lang in eng heb; do
  if echo "$list" | grep -qE "[[:space:]]base/assets/tessdata/${lang}\.traineddata$"; then
    size="$(echo "$list" | awk -v p="base/assets/tessdata/${lang}.traineddata" '$4==p{print $1}')"
...
ios="$(echo "$list" | grep -cE "[[:space:]]base/assets/ios/" || true)"
if [ "$ios" -gt 0 ]; then
