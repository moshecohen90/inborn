...
## Fixes round 26: the desktop shell never followed its own window (branch `desktop-pass-1`) — 22.9.2026
...
### F47 — the window opened, and reopened, under the minimum §9.7 gives it
...
The config was half of it. AppKit's `contentMinSize` constrains a drag and not a size set in code, and
...
own `Resized` event, reads the minimum from the same config the test asserts, and grows a window that is under
it. From a state file holding 500×400 the app now logs `window was 500x400, under the 1040x720 minimum; grown`
and comes up at 1040×720. Still unproven: that AppKit refuses a *drag* below it — that needs a pointer.
...
**F77** desktop Settings has none of §8.9's desktop rows (no model location, no Launch at login, no Advanced
section — so no backend picker, no VRAM offload, no local server). **F78** the desktop paywall's two buy
buttons are priced and permanently disabled, because the provider is `licence-key` and there is no in-app
checkout; the Work-first ordering §8.9 asks for is correct, so today it orders two dead cards first. **F79** a
document dropped on the window goes nowhere — `inborn:documents-dropped` is emitted, relayed and never
listened to, and folders are not handled at all. **F80** the native Mac app's screenshot-protection row reads
"Not available in a browser."

### Tests

10 new: 4 in `apps/mobile/test/desktopWindow.test.ts` (3 of them red on the old config), 3 in
`apps/mobile/src/lib/viewportResize.test.ts`, 3 in `desktopKeys.test.ts` (the double-fire one red without the
guard), plus 2 in `shell.rs`. `pn test` is 742 — core 521, mobile 200 (190 before this round), i18n 10, ui 11
— and `cargo test --release` 10.
