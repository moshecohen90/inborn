...
- **The site's default canonical is the real host (F237).** Also checked before fixing: `apps/site/dist` is **not**
  ...
  the build-time default. A default build now carries `inbornapp.com` in all 313 places and the staging host in none.
...
Gates: `pn typecheck` 0, `pn lint` 0, `pn check:store` PASS, **1,235 tests** (core 658, mobile 552, ui 13, i18n 12),
`pn web:build` + `pn web:smoke` PASS (the smoke now opens the strip's disclosure before reading the offline state),
`pnpm --filter @inborn/site check` "12 pages: no scripts, no external assets, no dead links, CSP present".
